...
Experigen.make_into_trial = function (that) {
	...
	that.makeScale = function(obj) {
		...
		for (var i=0; i<buttons.length; i+=1) {
			...
			// Tara TODO
			//  1) Update click handler so that in the event of an incorrect answer
			//     the user is required to listen again
			//  2) Only record the first answer provided (avoid recording correct answer
			//     if not given on first attempt)
