...
Experigen.make_into_trial = function (that) {
	...
	that.makeScale = function(obj) {
		...
		//
		// HACK: Alter the behavior per Tara McAllister
		//
		const soundButtonId = "#" + Experigen.screen().soundbuttons[0].id;
		const correctResponse = Experigen.screen().Correct_Response;

		// Override runextra so continue prompt always appears "Correct"
		runextra = "$('#prompt').html('Correct');";

		const clickHandlerString = function (i) {
			let str = "";
			str += "if (!Experigen.screen().recordResponseRecorded) {";
			str += " Experigen.screen().recordResponseRecorded = true;";
			str += " Experigen.screen().recordResponse(" + Experigen.screen().responses + "," + "'" + buttons[i] + "'" + ");";
			// str += " console.log('Response recorded (only first response recorded)');";
			str += "};";
			str += "if (!Experigen.screen().correctResponseAchieved) {";
			str += " if (" + i + " !== " + correctResponse + ") {";
			// str += "  console.log('Incorrect response, try again');";
			str += "  $('" + soundButtonId + "').val('Try Again" + Experigen.settings.strings.soundButton + "');";
			str += "  $('" + soundButtonId + "').prop('disabled', false);";
			str += " } else {";
			// str += "  console.log('Correct response, continue');";
			str += "  Experigen.screen().correctResponseAchieved = true;";
			str += "  $('" + soundButtonId + "').val('" + Experigen.settings.strings.soundButton + "');";
			str += "  $('" + soundButtonId + "').prop('disabled', true);";
			str += "  Experigen.screen().continueButtonClick(this, { hide: " + hide + ", disable: " + disable + " });";
			str += " };";
			str += "};";
			return str;
		}
		...
		for (var i=0; i<buttons.length; i+=1) {
			...
			// Replacement using HACK above
			str += '<input type="' + buttontype + '" value="' + buttons[i] + '" id="' + Experigen.screen().responses + 'button' + i + '" name="scale' + Experigen.screen().responses + '" class="scaleButton" onClick="' + clickHandlerString(i) + runextra;
