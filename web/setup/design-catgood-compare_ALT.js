...
Experigen.initialize = function () {
	...
	//this.blockToRun = -1;
// Deriving block number from the URL
	//var btrMatch = window.location.pathname.toString().match(/block([0-9]*)/);
	...
	// Deriving block and dump number from the URL
	var blocksPerDump = 4;
    var btrMatch = window.location.pathname.toString().match(/block([0-9]*)/);

    if(btrMatch.length < 2 || btrMatch[1].length < 1) {
        this.blockToRunInDump = "1";
        this.blockToRun = "1";
        console.error("Could not derive block to run based on URL! Defaulting to 1.");
    }
    else{
		this.blockToRunInDump = btrMatch[1];
		//Other version reads
        //this.blockToRun = parseInt(btrMatch[1],10);
		
		var dtrMatch = window.location.pathname.toString().match(/PTcontext([0-9]*)/);
        if(dtrMatch.length < 2 || dtrMatch[1].length < 1) {
            this.dumpNumber = "1";
            console.error("Could not derive dump to run based on URL! Defaulting to dump 1.");
        } 
        else {
            this.dumpNumber = dtrMatch[1];
        }
        
        this.blockToRun = (parseInt(this.blockToRunInDump, 10) +
        ((parseInt(this.dumpNumber, 10) - 1) * blocksPerDump)).toString();
  ...
  // if you want blocks:	
  var experimentBlock = []
  	.concat(items.subset("Block",this.blockToRunInDump.toString()))
  	.pairWith("view","stimulus_catgoodcompare_testing.ejs")						 
  	//.concat(catch_items.chooseRandom(nCatch) // 20
	//.pairWith("view","catch.ejs"))	
  
		//var experimentBlock = items.subset("Block",this.blockToRun+"")  
	...
	//var experimentBlock = []
	//	.concat(items.subset("Block",this.blockToRun.toString()))
	//	.pairWith("view","stimulus_catgoodcompare.ejs").shuffle();	
